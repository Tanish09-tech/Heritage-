...
export default function MasterMatchingView({ onSelectTradition, traditions, currentUser }) {
  ...
  const fetchApplications = async () => {
    try {
      const apps = await api.getApplications({ learnerId: learnerName });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-sans text-stone-900">
            Guru-Shishya Matchmaking & Apprenticeship
          </h1>
        ...
        <div className="flex bg-stone-100 p-1 rounded-xl w-fit shrink-0">
          ...
          >
            Find Masters ({MASTER_PRACTITIONERS.length})
          </button>
          <button
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="inline-flex items-center gap-1.5 text-[11px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md mt-1">
                        ...
                        <span>AI Match Score: {master.matchPercent}</span>
                      </div>
                    </div>
                  </div>
      ...
      {activeSubTab === 'MY_CONNECTIONS' && (
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-xs flex items-center justify-between">
            <div>
              <h3 className="font-bold text-sm text-stone-900">Apprenticeship Applications & Connections</h3>
              <p className="text-xs text-stone-500">Track your requests and chat with your accepted Gurus</p>
            </div>
          ...
          {applications.length === 0 ? (
            <div className="blueprint-card p-12 text-center text-stone-500 space-y-3">
              ...
              <p className="text-xs text-stone-500 max-w-md mx-auto">
                Explore available master practitioners under "Find Masters" to choose a Guru and send your apprenticeship request!
              </p>
              <button
                onClick={() => setActiveSubTab('FIND_MASTERS')}
                ...
                return (
                  ...
                  >
                    <div className="space-y-2">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-[#2e3e55] text-amber-300 font-bold flex items-center justify-center text-xs">
                          {app.practitionerName.slice(0, 2).toUpperCase()}
                        </div>
                        <div>
                    ...
                    <div className="flex items-center gap-3 self-end sm:self-center">
                      ...
                      >
                        <MessageSquare className="w-3.5 h-3.5" />
                        <span>{isAccepted ? 'Chat with Guru' : 'View Message / Status'}</span>
                      </button>
                    </div>
                  </div>
                );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-xs animate-fadeIn">
          <form onSubmit={handleSendRequestSubmit} className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-stone-200 p-6 space-y-4 text-stone-800">
            ...
            <div className="flex items-center gap-4 bg-emerald-50/70 p-3.5 rounded-2xl border border-emerald-200">
              ...
              <div>
                ...
                <span className="inline-block text-[10px] font-bold bg-white text-emerald-900 px-2 py-0.5 rounded border border-emerald-300 mt-1">
                  Match Score: {requestModalMaster.matchPercent}
                </span>
              </div>
            </div>
            ...
            <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 text-[11px] text-stone-500 flex items-start gap-2">
              ...
              <span>
                Once sent, your request will be queued for approval by {requestModalMaster.name}. Live chat will unlock immediately upon approval.
              </span>
            </div>
      ...
      {/* MODAL 2: Master Profile View Modal */}
      {selectedMasterModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-xs animate-fadeIn">
