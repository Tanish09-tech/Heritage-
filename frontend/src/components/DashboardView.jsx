...
import { Search, Filter, AlertTriangle, ShieldCheck, Activity, Users, MapPin, ChevronRight, BookOpen, Layers, Globe, GraduationCap, MessageSquare, CheckCircle2, Clock, XCircle, RefreshCw } from 'lucide-react';
...
export default function DashboardView({ traditions, onSelectTradition, onNavigateTab }) {
  ...
  const [rosterTab, setRosterTab] = useState('APPLICATIONS');
  ...
  const fetchRoster = async () => {
    ...
    try {
      const [apps, pracs, lrs] = await Promise.all([
        api.getApplications(),
      ...
      console.warn('Error fetching roster in DashboardView:', e);
    } finally {
  ...
  const filteredApps = applications.filter(app => {
    ...
    const matchesSearch = !rosterSearch || 
      app.learnerName?.toLowerCase().includes(rosterSearch.toLowerCase()) ||
      app.practitionerName?.toLowerCase().includes(rosterSearch.toLowerCase()) ||
      app.traditionName?.toLowerCase().includes(rosterSearch.toLowerCase()) ||
      app.note?.toLowerCase().includes(rosterSearch.toLowerCase());
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="p-4 space-y-3 flex-1 bg-white">
              ...
              <div className="bg-red-50/70 border border-red-200 rounded-xl p-2.5 text-xs text-red-900 space-y-0.5">
                ...
                <p className="line-clamp-2 text-xs text-red-950 font-medium">
                  {tradition.riskFactors ? tradition.riskFactors[0] : 'Transmission risk monitored'}
                </p>
              </div>
      ...
      {/* Admin Panel Roster Section for Guru-Shishya Matchmaking */}
      <div className="clean-card p-6 space-y-6 rounded-2xl border-2 border-indigo-900/10">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-[#0f2a4a] text-amber-300">
              <GraduationCap className="w-6 h-6" />
            </div>
            <div>
              <h3 className="font-cinzel text-lg font-bold text-slate-900">
                Guru-Shishya Active Roster & Apprenticeship Tracking
              </h3>
              <p className="text-xs text-slate-500">
                View all registered Gurus, Shishyas, incoming request notes, and approval statuses
              </p>
            </div>
          </div>
          <button
          ...
          >
            <RefreshCw className={`w-3.5 h-3.5 ${loadingRoster ? 'animate-spin text-amber-600' : ''}`} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            <span className="text-[11px] font-bold text-slate-500 uppercase block">Active Gurus</span>
            <span className="text-2xl font-black text-slate-900 mt-1 block">{practitioners.length}</span>
          </div>
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            <span className="text-[11px] font-bold text-slate-500 uppercase block">Registered Shishyas</span>
            <span className="text-2xl font-black text-slate-900 mt-1 block">{learners.length}</span>
          </div>
          <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-200">
            <span className="text-[11px] font-bold text-emerald-800 uppercase block">Approved Pairings</span>
            <span className="text-2xl font-black text-emerald-700 mt-1 block">{acceptedCount}</span>
          </div>
          <div className="bg-amber-50 p-4 rounded-xl border border-amber-200">
            <span className="text-[11px] font-bold text-amber-800 uppercase block">Pending Requests</span>
              ...
              {filteredApps.length === 0 ? (
                <tr>
                  <td colSpan="5" className="p-6 text-center text-slate-500">
                    No active apprenticeship applications found.
                  </td>
                </tr>
              ) : (
                filteredApps.map((app) => (
                  <tr key={app.id} className="hover:bg-slate-50 transition">
                    <td className="p-3 font-bold text-slate-900">{app.learnerName || 'Learner Shishya'}</td>
                    <td className="p-3 font-bold text-amber-900">{app.practitionerName || 'Master Guru'}</td>
                    <td className="p-3 font-semibold text-slate-700">{app.traditionName}</td>
                    <td className="p-3 max-w-xs">
                      <div className="p-2 bg-slate-50 rounded border border-slate-200 text-slate-600 text-[11px] italic">
                        "{app.note || app.message || 'Requesting mentorship.'}"
                      </div>
                    </td>
                    <td className="p-3">
                      {app.status === 'ACCEPTED' ? (
                        ...
                        <span className="px-2.5 py-1 rounded bg-red-100 text-red-800 font-bold text-[10px]">
                          DECLINED
                        </span>
                      ) : (
